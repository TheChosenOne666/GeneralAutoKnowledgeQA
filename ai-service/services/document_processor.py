...
class DocumentProcessor:
    ...
    async def process(
        ...
        对齐 WeKnora finalizing 语义：向量化完成并入库原始块后，文档**立即可被检索**，
        ...
        # 4. 原始块先入库 —— 向量一旦入库，文档即可被检索（对齐 WeKnora finalizing=queryable）
    ...
    async def _run_augment_task(self, task: dict) -> None:
        """持久化队列消费：执行单文档问答增强并回调 ready（对齐 WeKnora finalizing 队列任务）。
    ...
    async def _generate_qa_augment(
        ...
        对齐 WeKnora：每批并发生成（限并发度）+ 批量 embedding（一次调用），而非单线程串行。
        best-effort：LLM 不可用 / 配置错误 / 单块超时，跳过剩余增强（文档仍就绪可用），
        不阻塞主流程。
        """
        selected = chunks[: settings.qa_max_pairs]
        augmented: list[dict] = []
